...
const MemoriesGallery = () => {
  ...
  return (
    <section className="relative py-16 px-6 bg-[url('https://www.transparenttextures.com/patterns/cork-board.png')] dark:bg-gray-900 flex justify-center overflow-hidden h-[750px]">
      ...
      {/* Title */}
      <h2 className="absolute top-8 left-1/2 transform -translate-x-1/2 text-5xl font-extrabold text-gray-900 dark:text-gray-100 drop-shadow-md text-center">
        Reliving the <span className="text-ieee-blue">Memories</span> ✨
      </h2>

      {/* Nostalgic Subtext */}
      <p className="absolute top-20 left-1/2 transform -translate-x-1/2 italic text-lg text-gray-700 dark:text-gray-300 text-center">
      ...
      <style jsx>{`
        @media (max-width: 768px) {
          /* Adjust heading and subheading spacing */
          h2 {
            font-size: 2rem;
            top: 5%;
          }
...
const MemoryCard = ({ src, caption, top, left, rotate, isBig }) => {
  return (
    <motion.div
      ...
      className={`absolute memory-card ${isBig ? "scale-110" : ""}`}
